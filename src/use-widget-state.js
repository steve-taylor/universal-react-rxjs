...
export default function useWidgetState(context, compare) {
    ...
    if (phase === SERVER) {
        ...
        const [state] = useState(() => {
            ...
            // If we're *really* server-side, data$ is generated by observableOf from the initial value produced by
        ...
        const [state, setState] = useState(() => {
            ...
            subscription.current = withSkipDuplicates(data$, compare).subscribe((value) => {
...
function withSkipDuplicates(data$, compare) {
    ...
    if (Array.isArray(compare)) {
        return data$.pipe(
            distinctUntilChanged((a = {}, b = {}) => compare.every((property) => a[property] === b[property]))
        );
    }

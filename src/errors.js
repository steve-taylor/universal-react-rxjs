...
export function noImmediateStateOnHydrationError(name, elementId) {
    return (
        ...
        + ' To avoid this error, ensure getData(props$, hydration, immediate) returns an RxJS Observable which produces an event'
...
export function noImmediateStateOnRenderError(name) {
    return (
        ...
        + ' error, ensure getData(props$, hydration, immediate) returns an RxJS Observable which produces an event immediately.'
    );
}
